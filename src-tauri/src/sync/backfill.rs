...
pub(super) fn backfill_schema(
    ...
    if needs_planner {
        changed |= backfill_planner(db_mutex, token)?;
    }
    if needs_music {
        changed |= backfill_music(db_mutex, token)?;
    }
    if needs_settings {
        changed |= backfill_settings(db_mutex, token)?;
    }
    if needs_planned_sessions {
        changed |= backfill_planned_sessions(db_mutex, token)?;
    }
...
fn backfill_planner(db_mutex: &Mutex<Db>, token: &str) -> Result<bool, String> {
...
fn backfill_music(db_mutex: &Mutex<Db>, token: &str) -> Result<bool, String> {
...
fn backfill_settings(db_mutex: &Mutex<Db>, token: &str) -> Result<bool, String> {
    ...
    match settings {
        Some(value) => {
            let db = db_mutex.lock().unwrap();
            db.upsert_user_settings_from_remote(&value, false)
                .map_err(|error| error.to_string())
        }
        None => Ok(false),
    }
...
fn backfill_planned_sessions(db_mutex: &Mutex<Db>, token: &str) -> Result<bool, String> {
...
fn backfill_logical_sessions(db_mutex: &Mutex<Db>, token: &str) -> Result<bool, String> {
