...
export function RecordSessionModal({ snapshot, anchorDay, initialRange, initialListId, onClose, onSave }: RecordSessionModalProps) {
  ...
  const [listId, setListId] = useState(initialListId || matchingList?.id || fallbackList?.id || "");
  const tasks = useMemo(() => eligibleTasks
    .filter((item) => item.listId === listId), [eligibleTasks, listId]);
