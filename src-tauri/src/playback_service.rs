...
pub(crate) fn do_play(state: &AppState, task_id: &str, trigger: &str) -> Result<(), String> {
    ...
    // The primary play control is a start/stop toggle. Stopping closes the
    // user-visible session so it cannot remain open across later days.
    if is_own(&run, &state.device_id)
        && run.active_task_id.as_deref() == Some(task_id)
        && run.phase.is_some()
    {
        drop(run);
        do_stop(state, TIMER_PAUSE_REASON_SAME_TASK_TOGGLE, trigger);
        return Ok(());
    }
    ...
    let mut nr = if baseline.active_session_id.is_some() {
        timer::resume(&baseline, task_id, now)
    } else {
